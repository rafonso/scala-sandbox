...
package p991 {
  ...
  class SudokuBoard[T](aboard: Array[Either[T, Set[T]]]) {
    ...
    val width = scala.math.sqrt(aboard.length).toInt
    val squareWidth = scala.math.sqrt(width).toInt
  ...
  object SudokuBoard {
    ...
    final def solve[T](b: SudokuBoard[T]): Option[SudokuBoard[T]] =
      b.board.zipWithIndex.find(_._1.isRight) match {
        ...
        case Some((Right(ns), i)) => ns.view.map { n =>
